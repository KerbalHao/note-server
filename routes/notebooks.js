/*
 * @Date: 2021-03-17 16:54:28
 * @LastEditors: KerbalHao
 * @FilePath: \server\routes\notebooks.js
 */
let express = require("express");
let { checkNotebook } = require("../helpers/check");
let { model } = require("../models");
const router = express.Router();

//获取笔记本列表
router.get("/", (req, res) => {
  model.Notebook.findAll({ where: { userId: req.session.user.id } }).then(
    (notebooks) => {
...
router.patch("/:notebookId", checkNotebook, (req, res) => {
  let title = req.body.title;
  model.Notebook.update(
    { title: title },
    { where: { id: req.params.notebookId, userId: req.session.user.id } }
  ).then(([affectRow]) => {
    if (affectRow === 0) {
...
router.delete("/:notebookId", (req, res) => {
  model.Note.findAll({
    where: { notebookId: req.params.notebookId, userId: req.session.user.id }
  }).then((notes) => {
    if (notes.length > 0) {
      res
        .status(400)
        .send({ msg: "笔记本不为空或者回收站中还有属于当前笔记本的笔记" });
    } else {
      model.Notebook.destroy({
        where: { id: req.params.notebookId, userId: req.session.user.id }
      }).then((affectRow) => {
        if (affectRow === 0) {
          return res.status(400).send({ msg: "笔记本不存在" });
